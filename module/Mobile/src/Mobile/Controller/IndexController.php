        ...
        if ($request->isPost()) {
            ...
            if ($form->isValid()) {
               ...
               if (empty($dados['categoria']) || strtolower(trim($dados['categoria'])) === 'selecione') {
                ...
                if ($service->salvar()) {
                    // ===== INÍCIO DA ADIÇÃO PARA CORREÇÃO DO REDIRECIONAMENTO =====
                    // Força a escrita e o fechamento da sessão antes de redirecionar
                    // para garantir que a sessão não se perca.
                    try {
                        $sessionManager = $this->getServiceLocator()->get('Zend\Session\ManagerInterface');
                        if ($sessionManager) {
                            $sessionManager->writeClose();
                        }
                    } catch (\Exception $e) {
                        // Ignora o erro se o gerenciador não for encontrado para não quebrar a aplicação.
                    }
                    // ===== FIM DA ADIÇÃO =====
