    ...
    public function indexAction()
    {
        ...
        if (count($categorias)) {
            foreach ($lista as $item) {
                if (!in_array($item->getCategoria(), $categorias)) continue;
                $tratado[] = $item;
            }
            $lista = $tratado;
        }
        ...
        return new ViewModel(['lista' => $lista, 'service' => $service, 'form' => $form, 'sistema' => $nomeSistema, 'categoriasSelecionadas' => $categorias]);
    }
...
//                if(preg_match('/Atendimento especializado/',$categoriaTratada)) $categoriaTratada = utf8_encode('Aplicação - Atendimento especializado');
                ...
                /**
                 * Regra retirada a pedido de Jemima Mendes @ 31/10/2018
                 */
//                if ($categoriaTratada == 'Procedimentos de Aplicação - Não atendimento da solicitação de identificação pelo Nome Social') {
//                    $categoriaTratada = 'Procedimentos de Aplicação - Não atendimento da solicitação de identificação pelo Nome Social - travestis e transexuais';
//                }
                ...
                //CONDIÇÃO ESPECIAL APLICADA PARA O DIA DO ENEM
                /*
                if(!$alerta->getDiaAplicacao()){
                    $dia = ($alerta->getSistema() == 'Enem' && date('Y-m-d') == '2017-11-05') ? '1° dia' : '2° dia';
                }else{
                    $dia = preg_match('/1/', $alerta->getDiaAplicacao()) ? '1° dia' : '2° dia';
                }
                */

                if ($alerta->getDiaAplicacao() == '') {
                    $alerta->setDiaAplicacao('2° dia');
                }
                $dia = preg_match('/1/', $alerta->getDiaAplicacao()) ? '1° dia' : '2° dia';
                if (date('d/m/Y') == '11/11/2018') {
                    $dia = '2° dia';
                }

                //if($alerta->getSistema() == 'Enem') $dia = '2° dia'; //aplicado apenas no segundo dia de aplicação do Enem a pedido de alguem.
                ...
                $evento = [
                    ...
                    'customAttributes' => [
                        ...
                        'nivel_do_alerta_sgir' => ($alerta->getImpactoAplicacao() == 'Sim') ? '5 - Severo' : '2 - Atenção',
                        'sistema' => $alerta->getSistema(),
                        'ano_vigente' => date('Y'),
//                        'caminho_critico'=>'2. Implementação - Aplicação do exame', //Obsoleto
                        '__origem_do_alerta' => 'RM - Operação',
                        'etapas_do_cronograma' => 'Exame',
...
//                        'informacoes_adicionais'=>$alerta->getInformacoesAdicionais(),
            ...
            if (preg_match('/categoria_evento/', $e->getMessage())) {
                $error = 'A categoria informada é inválida, favor efetue a alteração para o valor correto.';
            }

            if ($codeEvent) {
                $reset = new Event();
                $reset->setCode($codeEvent);
                $reset->setAnonimo();
                $reset->cancelEvent();
            }
...
//                    'caminho_critico'=>'2. Implementação - Aplicação do exame', //Obsoleto
    ...
    public function updateExameAction()
    {
        $event = new Event();
        $event->setAnonimo();

        $filter = new Filter();
        $filter->where(Filter::F_GE, Filter::A_DATETIME, 'Created', '2017-10-26');
        ...
        foreach ($lista as $item) {
            $custom = $item->getCustomAttributes();
            if (isset($custom->categoria_evento) && $custom->categoria_evento != '') {
                $save->setCode($item->getCode());
                $customAttr = new CustomAttributes();
                $customAttr->set('etapas_do_cronograma', 'Exame');
                try {
                    $save->setCustomAttributes($customAttr);
                    $save->save();

                    echo 'Evento ' . $item->getCode() . ' Atualizado' . PHP_EOL;
                } catch (\Exception $e) {
                    echo 'Evento ' . $item->getCode() . ' Erro' . PHP_EOL;
                }
            }
        }
        die;
    ...
    public function testeAction()
    {
        ...
        debug(json_decode(file_get_contents("./data/json/dashboard_Enem.json"), true));
        die;
    }
} 